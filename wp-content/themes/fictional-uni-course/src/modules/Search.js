...
class Search {
  ...
  getResults() {
    $.getJSON(
      ...
      }/wp-json/university/v1/search?q=${this.searchInput.val()}`,
      (results) => {
        this.resultsContainer.html(`
                <div class="row">
                    <div class="one-third">
                    ...
                    <div class="one-third">
                        ...
                        ${
                          results.programs.length
                            ? `<ul class="link-list min-list">`
                            : `<p>No programs match that search.<a href="${universityData.root_url}/programs"> See all Programs</a></p>`
                        }
                            ${results.programs
                              .map(
                                (program) =>
                                  `<li><a href="${program?.permalink}">${program?.title}</a></li>`
                              )
                              .join('')}
                    ...
                    <div class="one-third">
                        ...
                        ${
                          results.campuses.length
                            ? `<ul class="link-list min-list">`
                            : `<p>No campuses match that search.<a href="${universityData.root_url}/campuses"> See all Campuses</a></p>`
                        }
                            ${results.campuses
                              .map(
                                (post) =>
                        ...
                        ${
                          results.events.length
                            ? ''
                            : `<p>No events match that search.<a href="${universityData.root_url}/events"> See all Events</a></p>`
                        }
                            ${results.events
                              .map(
                                (event) => `
